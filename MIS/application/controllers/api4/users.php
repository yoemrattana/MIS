...
class Users extends REST_Controller
{
	...
	private function mobile_auth($place_code, $imei)
	{
		...
		if ($query_from_hc->num_rows() > 0) {
			...
			$sql = "SELECT tblProvince.Name_Prov_K,
					...
					Name_Regional_K, Name_Unit_K, Name_Group_K,
					tblHFCodes.Name_Facility_K,
					...
					JOIN tblOD ON tblOD.Code_OD_T = tblHFCodes.Code_OD_T
					LEFT JOIN (
						  select Name_Regional_K, Name_Unit_K, Name_Group_K, Code_Group_T from tblMLGroup as a
						  join tblMLUnit as b on b.Code_Unit_T = a.Code_Unit_T
						  join tblMLRegion as c on c.Code_Regional_T = b.Code_Regional_T
					) as ML on tblCensusVillage.Code_ML = ML.Code_Group_T
	...
	public function permission_post()
    {
		...
		$p = $this->db->get('tblDevicePermission')->row_array();
		...
		$rs['send_phone'] = 1;
		$rs['hc_entery_followup_vmw'] = $p['HCEditFollowupVMW'];
