...
class MRRT_IPTf extends MY_Controller
{
    public function index()
	{
		if (!isset($_SESSION['permiss']['MRRT2'])) redirect('Home');

		$data['title'] = 'MRRT IPTf';
		$data['main'] = 'mrrt_iptf_view';
    ...
    public function getDetail()
    {
        ...
        $sql = "select * from tblMRRT_IPTf where Rec_ID = $recID";
    ...
    private function getMembers($parentID)
    {
        ...
        $sql = "select * from tblMRRT_IPTf_Member where Parent_ID = $parentID";
    ...
    private function insert($parent, $members)
    {
        ...
        $this->db->insert('tblMRRT_IPTf', $parent);
        ...
        if( !empty($members) ) {
            foreach($members as $r) {
                ...
                $this->db->insert('tblMRRT_IPTf_Member', $r);
            }
        }
    ...
    private function update($parent, $members, $parentId)
    {
        ...
        $this->db->update('tblMRRT_IPTf', $parent, ['Rec_ID' => $parentId]);
        ...
        if( !empty($members) ) {
            foreach($members as $r) {
                ...
                if ($recID > 0) {
                    $this->db->update('tblMRRT_IPTf_Member', $r, ['Rec_ID' => $recID]);
                } elseif ($recID < 0) {
                    $this->db->delete('tblMRRT_IPTf_Member', ['Rec_ID' => $recID * -1]);
                } else {
                    $this->db->insert('tblMRRT_IPTf_Member', $r);
                }
            }
