...
class IPT extends REST_Controller
{
	...
	public function form_get()
	{
		...
		$sql = "select a.Rec_ID as HouseMemberID,a.Name, a.Age, a.Sex, IPTDate, DoNotUse,Reject,
				Date,W1,W2,W3,W4, W1Specie,W2Specie,W3Specie,W4Specie,
				a.HouseHoldID,NotEnterForest,RefuseReason,Absent,iif(a.IPT = 'Yes' ,1,0) as IsIPT from
				(
					select * from tblLastmileHouseMember
				) as a
				left join
				(
					select * from tblLastmileIPT where IPTDate = '{$ipt_date}' and HouseHoldID = {$houseHoldID}
	...
	public function update_post()
	{
		...
		$where = [
			...
			//'Month' => date('m', strtotime($data[0]['IPTDate'])),
			//'Year' => date('Y', strtotime($data[0]['IPTDate'])),
		];
