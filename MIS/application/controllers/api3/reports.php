...
class Reports extends REST_Controller
{
		...
		for($i = 0; $i < $length; $i ++)
		{
			...
			if ($row['NumberTests'] > 0) {
				...
				if ($row['Diagnosis'] != 'N') {
					...
					$patientCode = isset($row['PatientCode']) ? $row['PatientCode'] : '';
					...
					$row['PatientCode'] = $this->MPatient->createCode($row['User_Code_Fa_T'], $row['Diagnosis'], $patientCode);
				} else {
		...
		for ($i = 0; $i < count($list); $i++) {
			...
			if ($data['NumberTests'] > 0) {
				...
				if($hasVMW) {
					...
					$this->MPatient->createHFNotification($data);
				}
	...
	public function search_patient_post()
	{
		...
		$sql = "select a.ID as Code_Vill_T, Year,Month,DateCase,NameK,Age,AgeType,Sex,PregnantMTHS,Weight,Temperature,Mobile,Diagnosis,Treatment
					  ,OtherTreatment,DOT3 'Dot3days',Refered,Dead,Remark,ID,1 as NumberTests,Rec_ID,Is_Mobile_Entry,Passive, PatientCode, PatientPhone
					  ,PQTreatment,G6PD,IsConsult,IsACT,IsPrimaquine,Primaquine15,Primaquine75,PrimaquineDate
					  ,G6PDHb as G6PDdL,G6PDdL as G6PDHb, Relapse,L1,LC,IMP,LC_Code,IMP_Text
				from tblVMWActivityCases as a
				join tblCensusVillage as b on a.ID = b.Code_Vill_T
				where Year = '$year' and Month = '$month' and (HCCode = '$hccode' or HCCode_Meeting = '$hccode') $where";
	...
	public function hc_data_post()
	{
		...
		$sql = "select Year,Month,DateCase,c.Code_Prov_T,c.Code_Dist_T,c.Code_Comm_T,a.Code_Vill_t,b.Code_Prov_T as PassProvince,b.Code_Dist_T as PassDistrict
					...
					,Treatment,OtherTreatment,Refered,Dead,ID,1 as NumberTests,Rec_ID,Is_Mobile_Entry,Weight, PatientCode, PatientPhone, G6PD, PQTreatment
	...
	public function get_reactive2_post()
	{
		$id = $this->post('Passive_Case_Id');
	...
	public function update_reactive2_post()
	{
