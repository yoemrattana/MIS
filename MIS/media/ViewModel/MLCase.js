function viewModel() {
	...
	self.rg.subscribe(function (code) {
		self.unitList(unitData.filter(r => r.rg == code));
	});
	...
	self.getReport = function () {
		...
		app.ajax('mlGetReport', submit).done(function (rs) {
	...
	self.showCaseEntryForm = function (model, event) {
		...
		sessionStorage.code_prov = '19';
		window.open('/CaseReport/' + 'vmw' + '?s=' + JSON.stringify(obj));
	};
	...
	self.editReport = function (model) {
		...
		if (model.has()) {
			...
			app.ajax('mlGetCase', submit).done(prepare);
		} else {
			prepare({ detail: [] });
		}
	...
	self.saveReport = function () {
		...
		app.ajax('mlUpdateCase', { submit: JSON.stringify(submit) }).done(function () {
	...
	self.deleteReport = function () {
		app.showDelete(function () {
			...
			app.ajax('mlDeleteReport', submit).done(function () {
