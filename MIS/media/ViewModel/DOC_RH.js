function viewModel() {
	...
	self.save = function () {
		...
		var submit = {
			...
			table: 'tblDoc' + self.menu().replace(' ', '').replace('-Accuracy', '').replace('PRH/', ''),
	...
	self.getDisable1 = function (sn, year) {
		if (sn >= 1 && sn <= 10 && year < 24) return true;
		if (sn == 13) return true;
		return false;
	};

	self.getDisable2 = function (sn, year) {
		if (sn >= 1 && sn <= 10 && year < 24) return true;
		if (sn == 13 && year < 24) return true;
		return false;
	};
}