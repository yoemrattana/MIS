function subViewModel(self) {
	...
	self.viewClick = function () {
		...
		app.ajax('/Lab/getReferenceSlide', submit).done(function (rs) {
			var head = rs.head || {};
			headID = head.Rec_ID;
			...
			for (var i = self.listModel().length; i < 5; i++) {
	...
	self.save = function () {
		...
		app.ajax('/Lab/saveReferenceSlide', submit).done(function (rs) {
			var head = rs.head || {};
			headID = head.Rec_ID;
	...
	self.getCounting = function (m) {
		if (isnullempty(m.ParasiteCount1()) || isnullempty(m.ParasiteCount2())) {
			m.Counting(null);
			return null;
		}
	...
	self.print = function () {
		open('/Lab/index/referenceslide_print/' + self.hc() + '/' + self.q());
	};
	...
	self.calcF = function () {
		var a = self.countTotal('Counting');
		var b = self.countTotal('DetectionA');
		if (b == 0) return '0%';
	...
	self.exportExcel = function () {
		if (self.pv() == null) {
			var submit = { q: self.year() };
			app.ajax('/Lab/getReferenceSlide', submit).done(function (rs) {
				rs.list.sortasc('Name_Prov_E', 'Name_OD_E', 'Name_Facility_E', 'Q');
				prepare(rs.list);
			});
		} else {
		...
		function prepare(arr) {
			...
			self.writeExcel(data, 'Reference Slide');
		}
	};
}