function viewModel() {
	...
	self.getDisable1 = function (sn, year) {
		...
		if (sn >= 9 && sn <= 27 && year < 24) return true;
		if (sn >= 39 && sn <= 42 && year < 24) return true;
		return false;
	};
	...
	self.getDisable2 = function (sn, year) {
		...
		if (sn >= 9 && sn <= 26 && year < 24) return true;
		if (sn == 27 && year < 22) return true;
		if (sn >= 39 && sn <= 42 && year < 24) return true;
		return false;
	};
}