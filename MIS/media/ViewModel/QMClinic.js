function subViewModel(self) {
	...
	self.chboxUnknownChange = function (model, event) {
		var val = model[event.currentTarget.name];
		var verifier = event.currentTarget.name.slice(0, -7);
		if (val()) model[verifier]('');
	};
	...
	self.save = function () {
		...
		if (model.Age() != '' && model.Age() < 2) {
			scrollToElement(model.Age.element);
			app.showMsg('Incorrect Data', 'Please input Age >= 2');
			return;
		}
		if (model.Temp() != '' && (model.Temp() < 30 || model.Temp() > 45)) {
